...
async function getContentInfo(slug, type = "movie") {
  ...
  const poster = getAbsoluteUrl(
    $("img[src*='/poster/']").attr("src") ||
    $("img[data-src*='/poster/']").attr("data-src") ||
    $("img.img-fluid").first().attr("src") ||
    $(".img-fluid.rounded").attr("src") ||
    ""
  );
 ...
 */
async function getEpisodeServers(serieSlug, seasonNumber = 1, episodeNumber = 1) {
  if (!serieSlug) {
    throw new ApiError(400, "El parametro serieSlug es requerido");
  }

  const sNum = seasonNumber || 1;
  const eNum = episodeNumber || 1;
  const url = `${BASE_URL}/serie/${serieSlug}/temporada/${sNum}/capitulo/${eNum}`;
  const html = await fetchHtml(url);
  const $ = cheerio.load(html);
